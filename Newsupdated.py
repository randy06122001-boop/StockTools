    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if self.in_news_table:
            if tag == "tr":
                ...
                if href:
                    self.current_cells.append(("link", href))
...
def fetch_stock_news(ticker: str, limit: int) -> list[NewsItem]:
    ...
    items = parser.items[:limit]
    if not items:
        raise RuntimeError(f"No news items were parsed for {ticker}. Ticker may not exist or Finviz may have changed its layout.")
    return items
...
def generate_lm_studio_summary(
    ...
    try:
        ...
        if "timed out" in str(exc).lower() or isinstance(exc, TimeoutError):
            raise RuntimeError(
                "LM Studio request timed out. The model may be too slow for this task. "
                "Try using a larger/faster model or reduce the number of headlines with --limit."
            ) from exc
        raise RuntimeError(
...
def build_report(items: list[NewsItem], generated_at: datetime, ai_summary: str | None = None, ticker: str | None = None) -> str:
    lines: list[str] = []
    if ticker:
...
def main() -> int:
    ...
    report = build_report(items, datetime.now().astimezone(), ai_summary=ai_summary, ticker=args.ticker)
    print(report)
